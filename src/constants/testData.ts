...
export const generateTestQuote = (customerIndex: number, addressIndex: number, itemIndex: number, serviceIndex: number) => {
  ...
  return {
    ...
    amount: baseAmount,
    amountWithTax,
    status: ['見積中', '回答済', '成約', '不成約', 'キャンセル'][Math.floor(Math.random() * 5)] as any,
    items,
    ...
    sourceType: ['syncmoving', 'suumo', '外部'][Math.floor(Math.random() * 3)] as any,
...
export const generateTestFormSubmission = (customerIndex: number, addressIndex: number, itemIndex: number, serviceIndex: number) => {
  ...
  return {
    ...
    status: ['pending', 'assigned', 'completed'][Math.floor(Math.random() * 3)] as any,
    truckAssignments: [],
    createdAt: new Date(Date.now() - Math.random() * 30 * 24 * 60 * 60 * 1000).toISOString(),
    contractStatus: Math.random() > 0.5 ? 'estimate' : 'confirmed' as any,
    caseStatus: ['unanswered', 'answered', 'contracted', 'lost', 'cancelled'][Math.floor(Math.random() * 5)] as any,
