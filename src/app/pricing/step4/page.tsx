...
export default function PricingStep4Page() {
  ...
  // 初期データの読み込み
  useEffect(() => {
    const savedRules = localStorage.getItem('pricingStep4');
    if (savedRules) {
  ...
  useEffect(() => {
    if (!isLoading) {
      localStorage.setItem('pricingStep4', JSON.stringify(seasonRules));
    }
